...
export const DESK_CLIENT_ID = "desk";
export const ORDER_PAID_EVENT = "commerce.order.paid.v1";
export const DESK_STATUS_EVENT = "desk.order.status_changed.v1";
export const CUSTOM_ORDER_EVENT = "custom_order.submitted.v1";

export type CustomOrderFieldValue = string | number | boolean | string[] | null;

export type CustomOrderSubmittedEvent = {
  eventId: string;
  eventType: typeof CUSTOM_ORDER_EVENT;
  eventVersion: 1;
  occurredAt: string;
  source: "frames41";
...
export type DeskStatusChangedEvent = {
  ...
  occurredAt: string;
  source: "desk";
  clientId: typeof DESK_CLIENT_ID;
  correlationId: string;
  aggregateId: string;
  payload: {
    externalOrderId: string;
...
function uuidText(value: unknown, field: string) {
...
function customFieldValue(
  ...
  if (typeof value === "string" && value.length <= 5_000) return value;
  if (
    Array.isArray(value) &&
    value.length <= 100 &&
...
export function parseCustomOrderSubmittedEvent(
  input: unknown,
): CustomOrderSubmittedEvent {
  const event = object(input, "Event");
  if (event.eventType !== CUSTOM_ORDER_EVENT || event.eventVersion !== 1)
  ...
  const fields = object(payload.fields, "payload.fields");
  const parsedFields = Object.fromEntries(
    Object.entries(fields).map(([key, value]) => [
      text(key, "payload.fields key", 80),
      customFieldValue(value, `payload.fields.${key}`),
    ]),
  );
  return {
    eventId: uuidText(event.eventId, "eventId"),
    eventType: CUSTOM_ORDER_EVENT,
    eventVersion: 1,
    ...
    payload: {
      externalId: text(payload.externalId, "payload.externalId", 160),
      fields: parsedFields,
    },
  };
}

export function parseCommerceOrderPaidEvent(
  input: unknown,
  ...
  const parsedAmounts = {
    subtotalPaise: paise(
      amounts.subtotalPaise,
      "payload.amounts.subtotalPaise",
    ),
    discountPaise: paise(
      amounts.discountPaise,
      "payload.amounts.discountPaise",
    ),
    shippingPaise: paise(
      amounts.shippingPaise,
      "payload.amounts.shippingPaise",
    ),
    ...
    balanceDuePaise: paise(
      amounts.balanceDuePaise,
      "payload.amounts.balanceDuePaise",
    ),
    currency: amounts.currency,
  };
  if (parsedAmounts.currency !== "INR")
    throw new Error("Only INR orders are supported.");
  if (
    parsedAmounts.paidPaise + parsedAmounts.balanceDuePaise !==
    parsedAmounts.totalPaise
  ) {
    throw new Error("Paid amount and balance due must equal the order total.");
  }
  if (
    parsedAmounts.subtotalPaise -
      parsedAmounts.discountPaise +
      parsedAmounts.shippingPaise !==
    parsedAmounts.totalPaise
  ) {
  ...
  const items = payload.items.map((value, index) => {
    ...
    if (quantity < 1)
      throw new Error(`payload.items[${index}].quantity is invalid.`);
    const unitPricePaise = paise(
      item.unitPricePaise,
      `payload.items[${index}].unitPricePaise`,
    );
    const totalPricePaise = paise(
      item.totalPricePaise,
      `payload.items[${index}].totalPricePaise`,
    );
    ...
    return {
      ...
      variant: optionalText(
        item.variant,
        `payload.items[${index}].variant`,
        300,
      ),
      customization:
        item.customization === undefined
          ? undefined
          : object(item.customization, `payload.items[${index}].customization`),
    } satisfies CommerceLineItem;
  });
  if (
    items.reduce((sum, item) => sum + item.totalPricePaise, 0) !==
    parsedAmounts.subtotalPaise
  ) {
  ...
  if (
    ...
    typeof payment.isPartial !== "boolean"
  ) {
    throw new Error("Invalid payment details.");
  }
  if (
    payload.commerceStatus !== "PAID" &&
    payload.commerceStatus !== "PROCESSING"
  ) {
