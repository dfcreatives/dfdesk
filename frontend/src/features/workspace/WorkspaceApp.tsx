...
type CollectionDetail =
  "Cash collections" | "UPI collections" | "Total collected" | "Outstanding";
...
export default function WorkspaceApp({
  ...
  const [showTeamAttendanceReport, setShowTeamAttendanceReport] =
    useState(false);
  ...
  const [markingAttendanceFor, setMarkingAttendanceFor] = useState<
    string | null
  >(null);
  ...
  const [orderCustomFields, setOrderCustomFields] = useState<
    Record<string, OrderCustomFieldValue>
  >({});
  const [orderFieldDefinitions, setOrderFieldDefinitions] = useState<
    OrderFieldDefinition[]
  >([]);
  const [showOrderFields, setShowOrderFields] = useState(false);
  const [editingOrderField, setEditingOrderField] =
    useState<OrderFieldDefinition | null>(null);
  const [fieldLabel, setFieldLabel] = useState("");
  const [fieldType, setFieldType] = useState<OrderFieldType>("TEXT");
  ...
  const [connectedWebsiteName, setConnectedWebsiteName] = useState("Commerce");
  ...
  useEffect(() => {
    ...
    apiRequest<{
      ...
      .then((response) => {
        ...
        } else if (response.user && window.location.pathname !== "/overview") {
  ...
  useEffect(() => {
    ...
    const loadConnection = () => {
      apiRequest<{ connected: boolean; websiteName: string | null }>(
        "/api/v1/integrations/connection",
        { cache: "no-store" },
      )
        .then((response) => {
          if (!cancelled && response.websiteName) {
            setConnectedWebsiteName(response.websiteName);
          }
        })
        .catch(() => undefined);
    };
    loadConnection();
    const interval = window.setInterval(loadConnection, 30_000);
    return () => {
      cancelled = true;
  ...
  useEffect(() => {
    if (!currentUser) return;
    let cancelled = false;
    apiRequest<{ fields: OrderFieldDefinition[] }>("/api/v1/order-fields", {
      cache: "no-store",
    })
      .then(({ fields }) => {
        if (!cancelled) setOrderFieldDefinitions(fields);
      })
      .catch((error: Error) => {
  ...
  useEffect(() => {
    ...
    if (
      !canRefreshTeam ||
      (activeNav !== "Overview" && !showTeamAttendanceReport)
    ) {
      return;
    }
    ...
    const refreshTeamAttendance = () => {
      apiRequest<{ workspace: Workspace }>("/api/v1/attendance", {
        cache: "no-store",
        signal: controller.signal,
      })
        .then(({ workspace }) => {
          ...
          setEmployeeData((current) => {
            ...
            const next = current.map((employee) => {
              ...
              const attendanceChanged =
                employee.attendanceSeconds !==
                  serverEmployee.attendanceSeconds ||
    ...
    const interval = window.setInterval(refreshTeamAttendance, 30_000);
  ...
  async function saveAccountCredentials() {
    if (!currentUser || currentUser.role === "Employee" || accountSaving)
      return;
  ...
  function saveTask() {
    ...
    if (
      ...
      collaborators.some((collaborator) => !collaborator?.responsibility)
    ) {
  ...
  function getTouchDropTarget(clientX: number, clientY: number) {
    ...
    return element?.closest<HTMLElement>("[data-task-drop-target]")?.dataset
      .taskDropTarget;
  }
  ...
  function openOrderFieldEditor(field?: OrderFieldDefinition) {
    setEditingOrderField(field ?? null);
    setFieldLabel(field?.label ?? "");
    setFieldType(field?.type ?? "TEXT");
    setFieldRequired(field?.required ?? false);
    setFieldShowInList(field?.showInList ?? true);
  ...
  async function reloadOrderFields() {
    const response = await apiRequest<{ fields: OrderFieldDefinition[] }>(
      "/api/v1/order-fields",
      { cache: "no-store" },
    );
    setOrderFieldDefinitions(response.fields);
  }

  async function saveOrderField() {
    ...
    try {
      await apiRequest(
        editingOrderField
          ? `/api/v1/order-fields/${editingOrderField.id}`
          : "/api/v1/order-fields",
        {
          method: editingOrderField ? "PATCH" : "POST",
          body: JSON.stringify({
            label,
            type: fieldType,
            ...
            helpText: fieldHelpText.trim() || null,
            options,
          }),
        },
      );
      await reloadOrderFields();
  ...
  async function archiveOrderField(field: OrderFieldDefinition) {
    if (!window.confirm(`Remove the “${field.label}” field from the form?`))
      return;
    try {
      await apiRequest(`/api/v1/order-fields/${field.id}`, {
        method: "DELETE",
      });
      await reloadOrderFields();
      if (editingOrderField?.id === field.id) openOrderFieldEditor();
    } catch (error) {
      setFieldError(
        error instanceof Error ? error.message : "Unable to remove the field.",
      );
    }
  }

  async function moveOrderField(field: OrderFieldDefinition, offset: -1 | 1) {
    const index = orderFieldDefinitions.findIndex(
      (definition) => definition.id === field.id,
    );
    const neighbor = orderFieldDefinitions[index + offset];
    if (!neighbor) return;
    try {
      await Promise.all([
        apiRequest(`/api/v1/order-fields/${field.id}`, {
          method: "PATCH",
          body: JSON.stringify({ position: neighbor.position }),
        }),
        apiRequest(`/api/v1/order-fields/${neighbor.id}`, {
          method: "PATCH",
  ...
  function openOrderForm(order?: Order) {
    ...
    setOrderCustomFields(order?.customFields ?? {});
    setOrderError("");
    setShowOrderForm(true);
  }

  function saveOrder() {
    if (!orderFieldDefinitions.length) {
      setOrderError("Add at least one custom field before creating an order.");
      return;
    }
    const missingField = orderFieldDefinitions.find((field) => {
      if (!field.required) return false;
      const value = orderCustomFields[field.key];
      return (
        value === undefined ||
        value === null ||
        value === "" ||
    ...
    if (missingField) {
      setOrderError(`${missingField.label} is required.`);
      return;
    }
    const createdAt = editingOrder?.createdAt ?? Date.now();
    const order: Order = editingOrder
      ? { ...editingOrder, customFields: orderCustomFields }
      : {
          id: `#CO-${String(Date.now()).slice(-8)}`,
          createdAt,
          customer: "Custom order",
          item: "Custom form submission",
  ...
  function deleteOrder(order: Order) {
    ...
    setOrderData((current) => current.filter((entry) => entry.id !== order.id));
    setTaskData((current) =>
  ...
  const visibleOrders = orderData.filter((order) =>
    `${order.id} ${Object.values(order.customFields ?? {})
      .flat()
      .join(" ")}`
      .toLowerCase()
      .includes(searchQuery.toLowerCase()),
  );
  const listedOrderFields = orderFieldDefinitions.filter(
    (field) => field.showInList,
  );
  const visibleOrderFields = listedOrderFields.length
    ? listedOrderFields
    : orderFieldDefinitions;
  const orderTableColumns = `110px ${visibleOrderFields
    .map(() => "minmax(120px, 1fr)")
    .join(" ")} 70px`;
  const orderTableMinWidth = Math.max(
    360,
    220 + visibleOrderFields.length * 136,
  );
  const formatCustomFieldValue = (
    field: OrderFieldDefinition,
    value: OrderCustomFieldValue | undefined,
  ) => {
  ...
  const outstandingOrderAmount = orderData.reduce(
    (total, order) =>
      total +
      Math.max(0, parseRupeesInput(order.value) - (order.advanceAmount ?? 0)),
    0,
  );
            ...
            orderId:
              ...
              orderData.find(
                (order) =>
                  ...
                  payment.customer.toLowerCase(),
              )?.id ??
              "Manual payment",
            customer: payment.customer,
            meta: `${payment.source === "Advance" ? "Order advance · " : ""}${payment.method} · ${new Intl.DateTimeFormat(
              "en-IN",
              {
                day: "2-digit",
                month: "short",
  ...
  const splitPaymentTotal =
    parseRupeesInput(cashPaymentAmount) + parseRupeesInput(upiPaymentAmount);
    ...
    >
      ...
      <aside className={`sidebar ${mobileMenuOpen ? "mobile-open" : ""}`}>
        ...
        <div className="workspace-switcher">
          ...
          <span>
            <strong>{connectedWebsiteName}</strong>
              ...
              <section className="task-bento-grid" aria-label="Employee tasks">
                {currentUser.role !== "Employee" && (
                  <article
                    ...
                    onDragOver={(event) => {
                      if (currentUser.role !== "Manager" || !draggedTaskTitle)
                        return;
                      event.preventDefault();
          ...
          <div className="page-wrap orders-crud-page">
            <section className="orders-crud-header">
              <div>
                ...
                <h1>Orders</h1>
                <p className="subcopy">
                  Every order detail is defined by your custom form.
                </p>
              </div>
              <div className="orders-crud-header-actions">
                {currentUser.role !== "Employee" && (
                  <button
                    className="button"
                    onClick={() => {
                      openOrderFieldEditor();
                ...
                <button
                  className="button button-primary"
                  onClick={() => {
                    if (orderFieldDefinitions.length) openOrderForm();
                    else {
                      openOrderFieldEditor();
                      setShowOrderFields(true);
                    }
            ...
            <section className="panel orders-crud-panel">
              ...
              <div className="orders-crud-table">
                <div
                  className="orders-crud-row orders-crud-table-head"
                  style={{
                    gridTemplateColumns: orderTableColumns,
                    ...
                    >
                      <strong className="order-id-source">
                        ...
                        {order.source === "Frames 41" && (
                          <small>Frames 41</small>
                        )}
                      </strong>
                      {visibleOrderFields.map((field) => (
                        <span className="custom-field-cell" key={field.id}>
                    ...
                    contextItems.map((item) => (
                      ...
                      >
                        <div
                          ...
                          role={
                            activeNav === "Collections" ? "button" : undefined
                          }
                            ...
                            <div className="collection-detail-panel">
                              ...
                              {collectionDetailRows.length ? (
                                ...
                                <p className="collection-detail-empty">
                                  No details for this{" "}
                                  {collectionReportPeriod.toLowerCase()} report.
                                </p>
                              )}
                            </div>
                          )}
                  ...
                  showAccountCredentials && (
                    ...
                    >
                      <div className="settings-account-heading">
                        ...
                        <p>
                          Update your account name or password. Changes also
                          appear in Admins &amp; managers.
                        </p>
                      </div>
                      ...
                      {accountError && (
                        <p
                          className="settings-account-message error"
                          role="alert"
                        >
                          {accountError}
                        </p>
                      ...
                      {accountMessage && (
                        <p
                          className="settings-account-message success"
                          role="status"
                        >
                          {accountMessage}
                        </p>
                ...
                <div className="overview-primary-grid">
                  ...
                  >
                    <div
                      ...
                      onKeyDown={(event) =>
                        handleMetricKeyDown(event, "Reports")
                      }
                    >
                      ...
                      <p>
                        Hours logged ·{" "}
                        {reportPeriodLabels[collectionReportPeriod]}
                      </p>
                    ...
                    {currentUser.role === "Employee" && (
                      <div className="metric-card employee-attendance-card">
                        ...
                        <p>Attendance</p>
                        <strong>
                          {formatAttendance(currentAttendanceSeconds)}
                        </strong>
                        <small>
                    ...
                    <div
                      ...
                      onKeyDown={(event) =>
                        handleMetricKeyDown(event, "Orders")
                      }
                    >
                      <div className="metric-top">
                      ...
                      <p>Active orders</p>
                      <strong>
                        {reportingOrders.length.toString().padStart(2, "0")}
                      </strong>
                      <small>
                        {reportPeriodLabels[collectionReportPeriod]}
                      </small>
                    ...
                    <div
                      ...
                      onKeyDown={(event) =>
                        handleMetricKeyDown(event, "Collections")
                      }
                    >
                      <div className="metric-top">
                      ...
                      <p>
                        Collected · {reportPeriodLabels[collectionReportPeriod]}
                      </p>
                      <strong>
                        {formatRupees(
                          reportCashCollectedAmount + reportUpiCollectedAmount,
                ...
                <section className="content-grid overview-team-grid">
                  <div className="panel team-panel">
                    ...
                    <div className="team-summary">
                      ...
                      <div>
                        <strong>
                          {formatAttendance(teamTodayAverageSeconds)}
                        </strong>
          ...
          >
            ...
            >
              {(["Daily", "Weekly", "Monthly"] as AttendanceReportPeriod[]).map(
                (period) => (
                  <button
                    className={
                      attendanceReportPeriod === period ? "selected" : ""
                    }
                    key={period}
            ...
            <div className="attendance-report-summary">
              <div>
                ...
                <strong>
                  {formatAttendance(attendanceReportTotalSeconds)}
                </strong>
              </div>
              <div>
                <small>Average hours / day</small>
                <strong>
                  {formatAttendance(attendanceReportAverageSeconds)}
                </strong>
              </div>
              <div>
                ...
                {attendanceReportDays.length ? (
                  attendanceReportDays.map((day) => (
                    <div className="attendance-report-row" key={day.date}>
                      ...
                      <strong>{formatAttendance(day.durationSeconds)}</strong>
                    </div>
                  ))
                ) : (
                  <p className="attendance-report-empty">
                    No attendance was recorded for this{" "}
                    {attendanceReportPeriod.toLowerCase()} period.
                  </p>
                )}
              </div>
            </div>
      ...
      {showTeamAttendanceReport && canViewTeamAttendance && (
        ...
        >
          ...
          >
            <div className="modal-header">
              <div>
                <h2 id="team-attendance-report-title">
                  Team attendance report
                </h2>
                <p>
                  Every employee&apos;s clock times and hours for{" "}
                  {reportPeriodLabels[teamAttendancePeriod].toLowerCase()}.
                </p>
              </div>
              <button
                className="icon-button"
            ...
            >
              {(["Daily", "Weekly", "Monthly"] as AttendanceReportPeriod[]).map(
                (period) => (
                  <button
                    className={
                      teamAttendancePeriod === period ? "selected" : ""
                    }
                    key={period}
                    onClick={() => {
                      setTeamAttendanceError("");
                    ...
                    return (
                      ...
                      >
                        <span className="team-attendance-person">
                          <span
                            className={`avatar avatar-${row.employee.color}`}
                          >
                        ...
                        {canMarkTeamAttendance &&
                          row.date &&
                          row.clockIn !== null && (
                            <span className="attendance-auto-mark">
                              Auto present
                            </span>
                          )}
                        {canMarkTeamAttendance &&
          ...
          >
            <div className="modal-header">
              <div>
                ...
                <p>
                  Choose the lead and split responsibilities across
                  collaborators.
                </p>
              </div>
              <button
            ...
            {taskOwner &&
              employeeData.some((employee) => employee.id !== taskOwner) && (
                <fieldset className="task-collaborator-fieldset">
                  ...
                  <p>
                    Select everyone sharing this task and describe their part.
                  </p>
                  <div className="task-collaborator-list">
                    {employeeData
                      ...
                      .map((employee) => {
                        ...
                        return (
                          <div
                            className="task-collaborator-row"
                            key={employee.id}
                          >
                            <label>
                              <input
                              ...
                              />
                              <span
                                className={`avatar avatar-${employee.color}`}
                              >
                                {employee.initials}
                              </span>
                              <span>
            ...
            {taskFormError && (
              <p className="task-form-error">{taskFormError}</p>
            )}
            <button
              className="button button-primary modal-submit"
              onClick={saveTask}
      ...
      {showOrderFields && (
        <div
          className="modal-backdrop"
          role="presentation"
          onClick={() => setShowOrderFields(false)}
        >
          <div
            className="payment-modal order-fields-modal"
            role="dialog"
            aria-modal="true"
            aria-labelledby="order-fields-title"
            onClick={(event) => event.stopPropagation()}
          >
            <div className="modal-header">
              <div>
                <span className="modal-kicker">Order management</span>
                <h2 id="order-fields-title">Customize order fields</h2>
                <p>Add workspace-specific information to every order.</p>
              </div>
              <button
                className="icon-button"
                onClick={() => setShowOrderFields(false)}
            ...
            <div className="order-field-manager">
              <div className="order-field-list">
                {orderFieldDefinitions.length ? (
                  orderFieldDefinitions.map((field, index) => (
                    <article
                      className={
                        editingOrderField?.id === field.id ? "selected" : ""
                      }
                      key={field.id}
                    >
                      <button
                        className="order-field-select"
                        onClick={() => openOrderFieldEditor(field)}
                      >
                        <strong>{field.label}</strong>
                        <small>
                          {field.type.toLowerCase().replace("_", " ")}
                          {field.required ? " · Required" : ""}
                          {field.showInList ? " · List column" : ""}
                        </small>
                      </button>
                      <span className="order-field-actions">
                        <button
                          className="icon-button"
                          disabled={index === 0}
                          onClick={() => void moveOrderField(field, -1)}
                        ...
                        <button
                          className="icon-button"
                          disabled={index === orderFieldDefinitions.length - 1}
                          onClick={() => void moveOrderField(field, 1)}
                        ...
                        <button
                          className="icon-button danger-button"
                          onClick={() => void archiveOrderField(field)}
                  ...
                  <div className="order-fields-empty">
                    <SlidersHorizontal size={20} />
                    <strong>No custom fields yet</strong>
                    <small>Create the first field using the form below.</small>
                  </div>
                )}
              </div>
              <div className="order-field-editor">
                <div className="order-field-editor-title">
                  <strong>
                    {editingOrderField ? "Edit field" : "Add a custom field"}
                  </strong>
                  {editingOrderField && (
                    <button
                      className="text-button"
                      onClick={() => openOrderFieldEditor()}
                    >
                      Add new instead
                    </button>
                  )}
                ...
                <label className="input-label">
                  ...
                  <select
                    ...
                    value={fieldType}
                    onChange={(event) =>
                      setFieldType(event.target.value as OrderFieldType)
                    }
                  >
                ...
                <label className="input-label">
                  ...
                  <input
                    className="form-input"
                    value={fieldHelpText}
                    onChange={(event) => setFieldHelpText(event.target.value)}
                  />
                </label>
                <div className="order-field-switches">
                  <label>
                    <input
                      type="checkbox"
                  ...
                  <label>
                    <input
                      ...
                      onChange={(event) =>
                        setFieldShowInList(event.target.checked)
                      }
                    />
                    Show in order list
                  </label>
                </div>
                {fieldError && <p className="payment-error">{fieldError}</p>}
                <button
                  className="button button-primary modal-submit"
                  disabled={fieldSaving}
                  onClick={() => void saveOrderField()}
                >
                  <Check size={16} />
                  {fieldSaving
                    ? "Saving…"
      ...
      {showOrderForm && (
        ...
        >
          <div
            className="payment-modal custom-order-modal"
            role="dialog"
            aria-modal="true"
          ...
          >
            <div className="modal-header">
              <div>
                ...
                <p>Every detail below is defined by your custom form.</p>
              </div>
              <button
                className="icon-button"
                onClick={() => setShowOrderForm(false)}
            ...
            {orderFieldDefinitions.length > 0 ? (
              <section className="order-custom-fields">
                <div className="order-custom-fields-heading">
                  <span className="modal-kicker">Order details</span>
                  <small>Fully generated from your custom form</small>
                </div>
                {orderFieldDefinitions.map((field) => {
                  const value = orderCustomFields[field.key];
                  const setValue = (next: OrderCustomFieldValue) => {
                    setOrderCustomFields((current) => ({
                      ...current,
                      [field.key]: next,
                    }));
                    setOrderError("");
                  ...
                  if (field.type === "CHECKBOX")
                    return (
                      <label className="custom-checkbox-field" key={field.id}>
                        <input
                          type="checkbox"
                          checked={value === true}
                  ...
                  if (field.type === "MULTI_SELECT") {
                    ...
                    return (
                      <fieldset className="custom-multi-field" key={field.id}>
                        <legend>
                          {field.label}
                          {field.required ? " *" : ""}
                        </legend>
                  ...
                  if (field.type === "SELECT")
                    return (
                      <label className="input-label" key={field.id}>
                        {field.label}
                        {field.required ? " *" : ""}
                        <select
                          className="form-input"
                        ...
                        {field.helpText && (
                          <small className="form-help">{field.helpText}</small>
                        )}
                      </label>
                    );
              ...
              <div className="orders-crud-empty custom-order-empty">
                <SlidersHorizontal size={20} />
                <strong>No form fields configured</strong>
                <small>Close this form and customize fields first.</small>
              </div>
            )}
            ...
            <button
              className="button button-primary modal-submit"
              onClick={saveOrder}
              disabled={!orderFieldDefinitions.length}
            >
              {editingOrder ? <Pencil size={15} /> : <Plus size={16} />}
              {editingOrder ? "Update custom order" : "Create custom order"}
            </button>
          </div>
        </div>
      )}
